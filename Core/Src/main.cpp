...
osMessageQId Display_Event = 0;
struct netif gnetif; /* network interface structure */
ip_addr_t ip_addr_remote;
...
void  vTaskGUI(void *pvParameters)
{
	...
	while(1)
	{
		event = osMessageGet(Display_Event, 0);
		if( event.status == osEventMessage )
		{
			switch(event.value.v)
			{
				case Initial_Message:
					GUI_DispString("State: Ethernet Initialization ...\n");
					break;
				case DHCP_In_Process:
					GUI_DispString("State: Looking for DHCP Server ...\n");
					break;
				case DHCP_Get_Ip:
					GUI_DispString("IP address assigned by a DHCP server: ");
					GUI_DispString(ip4addr_ntoa((const ip4_addr_t *)&gnetif.ip_addr));
					GUI_DispString("\n");
					break;
				case DHCP_Fail:
					GUI_DispString("DHCP searching failed\n");
					break;
				case Cable_Fail:
					GUI_DispString("Please plug cable\n");
					break;
				...
				case Remote_Disconnected:
					...
					GUI_DispString("Http lost package code:0x07\n");

					break;
				case Remote_Disconnected1:

					GUI_DispString("Http lost package code:0x08\n");

					break;
				case Remote_Disconnected2:

					GUI_DispString("Http lost package code:0x09\n");
...
static void StartThread(void const * argument)
{
	...
	osMessagePut(Display_Event, Initial_Message, NULL);
...
//	/* Notify user about the network interface config */
	...
	osMessagePut(Display_Event, DHCP_In_Process, NULL);
	#endif
...
static void AppTaskCreate(void)
{
	...
	osMessageQDef(Display_Queue, 3, uint16_t);
	Display_Event = osMessageCreate (osMessageQ(Display_Queue), NULL);
